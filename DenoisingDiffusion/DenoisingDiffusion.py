...
def main():
	...
	fliResultImage0 = CFLImage()
	fliResultImage1 = CFLImage()
	fliResultImage2 = CFLImage()

	# 이미지 뷰 선언 # Declare the image view
	viewImageLearn = CGUIViewImage()
	viewImageValidate = CGUIViewImage()
	viewImageResult0 = CGUIViewImage()
	viewImageResult1 = CGUIViewImage()
	viewImageResult2 = CGUIViewImage()
	...
	while True:
		...
		if (res := fliValidateImage.Load('../../ExampleImages/GenerativeAdversarialNetwork/mnist100.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageResult0.SetImagePtr(fliResultImage0)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break

		# Result 이미지 뷰에 이미지를 디스플레이 # Display the image in the source image view
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageResult1.SetImagePtr(fliResultImage1)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break

		# Result 이미지 뷰에 이미지를 디스플레이 # Display the image in the source image view
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageResult2.SetImagePtr(fliResultImage2)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break
		...
		layerResult0 = viewImageResult0.GetLayer(0)
		layerResult1 = viewImageResult1.GetLayer(0)
		layerResult2 = viewImageResult2.GetLayer(0)
		...
		# 객체 생성 # Create object
		denoisingDiffusionDL = CDenoisingDiffusionDL()

		# 학습할 이미지 설정 # Set the image to learn
		denoisingDiffusionDL.SetLearningImage(fliLearnImage)
		# 검증할 이미지 설정 # Set the image to validate
		denoisingDiffusionDL.SetLearningValidationImage(fliValidateImage)
		
		# 학습할 DenoisingDiffusion 모델 설정 # Set up the DenoisingDiffusion model to learn
		...
		while not denoisingDiffusionDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = denoisingDiffusionDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while True:
			...
			i32MaxIteration = denoisingDiffusionDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = denoisingDiffusionDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = denoisingDiffusionDL.GetLastEpoch()

			if i32Epoch != i32PrevEpoch and i32Iteration == i32MaxIteration and i32Epoch > 0:
				# 학습 결과 비용과 검증 결과 기록을 받아 그래프 뷰에 출력  
				# Get the history of cost and validation and print it at graph view
				listCosts = List[Single]()
				listSSIM = List[Single]()
				listPDV = List[Single]()
				listValidationEpoch = List[Int32]()

				res = denoisingDiffusionDL.GetLearningResultAllHistory(listCosts, listSSIM, listPDV, listValidationEpoch)[0]
				...
				if listCosts.Count != 0:
					...
					f32PDV = listPDV[listPDV.Count - 1] if listPDV.Count != 0 else 0

					# 해당 epoch의 비용과 검증 결과 값 출력 # Print cost and validation value for the relevant epoch
					print("Cost : {:.6f} SSIM : {:.6f} PDV : {:.6f} Epoch {} / {}".format(f32CurrCost, f32SSIM, f32PDV, i32Epoch, i32MaxEpoch))
					...
					if (listCosts.Count != 0 and i32PrevCostCount != listCosts.Count) or (listSSIM.Count != 0 and i32PrevValidationCount != listSSIM.Count):
						...
						i32Step = denoisingDiffusionDL.GetLearningValidationStep()
						listV1 = List[Single]()
						...
						for i in range(listPDV.Count - 1):
							listV2.Add((float)(i * i32Step))

						listV2.Add((float)(listCosts.Count - 1))
						# Graph View 데이터 입력 # Input Graph View Data
						viewGraph.Plot(listV2, listPDV, EChartType.Line, EColor.GREEN, "PDV")
			...
			if denoisingDiffusionDL.IsRunning() == False:
				break
		...
		denoisingDiffusionDL.SetInferenceResultImage(fliResultImage0)
		...
		# 생성할 이미지 설정 # Set the image to create
		denoisingDiffusionDL.SetInferenceResultImage(fliResultImage1)
		...
		# 생성할 이미지 설정 # Set the image to create
		denoisingDiffusionDL.SetInferenceResultImage(fliResultImage2)
		...
		while viewImageLearn.IsAvailable() and viewImageResult0.IsAvailable() and viewImageResult1.IsAvailable() and viewImageResult2.IsAvailable() and viewImageValidate.IsAvailable() and viewGraph.IsAvailable():
