...
def main():
    ...
    qrcodeDecoder = CMicroQRCodeDecoder()
    ...
    for i in range(i64Results):
        # MicroQRCode Decoder 결과를 얻어오기 위해 FLQuadD 선언 # Declare FLQuadD to retrieve the result from the MicroQRCode Decoder.
        flqRegion = CFLQuad[Double]()

        # MicroQRCode Decoder 결과들 중 Data Region 을 얻어옴 # Gets the Data Region from the MicroQRCode Decoder results.
        ...
        if (res := qrcodeDecoder.GetResultGridRegion(i, flfaGridRegion)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
        
        # QR Code Decoder 결과들 중 Finder Pattern을 얻어옴
        flfaFinderPattern = CFLFigureArray()
        
        if (res := qrcodeDecoder.GetResultFinderPattern(i, flfaFinderPattern)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
        
        strDecodedMsg = StringBuilder()
        # MicroQRCode Decoder 결과들 중 Decoded String 을 얻어옴 # Gets the decoded string from the results of the MicroQRCode Decoder.
        ...
        # MicroQRCode의 결과를 디스플레이 한다. # Display the result of the MicroQRCode.
        ...
        if (res := layer.DrawFigureImage(flfaFinderPattern, EColor.CYAN, 4)).IsFail():
            ErrorPrint(res, "Failed to draw figure.")
            return
