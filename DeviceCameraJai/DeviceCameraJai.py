...
def main():
	...
	# Jai 카메라 선언 // Declare the Jai camera
	camJai = CDeviceCameraJai()
	...
	while True:
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camJai.GetAutoDetectGigECameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camJai.GetAutoDetectUSBCameraSerialNumbers(listSerialNumbers)
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camJai.AutoDetectGigECamera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camJai.AutoDetectUSBCamera(i32SelectDevice)
		...
		if((er := camJai.Initialize()).IsFail()):
