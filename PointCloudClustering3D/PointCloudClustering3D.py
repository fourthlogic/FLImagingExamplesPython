...
def main():
	...
	while True:		
		...
		if(res := floSourceObject.Load("../../ExampleImages/Smoothing3D/Smoothing3DExample.ply")).IsFail() :		
		...
		pointCloudClustering3D = CPointCloudClustering3D()
		...
		pointCloudClustering3D.SetRadiusCoefficient(0.8)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if(res := pointCloudClustering3D.Execute()).IsFail() :	
			ErrorPrint(res, "Failed to execute Point Cloud Clustering 3D.")
			break
		...
		if(res := layer3DDst.DrawTextCanvas(flp, "Clustering Result", EColor.YELLOW, EColor.BLACK, 20)).IsFail() :		
			ErrorPrint(res, "Failed to draw text.\n")
			break
		
		# 3D 오브젝트 뷰에 결과 오브젝트 디스플레이
		i32ResultCount = pointCloudClustering3D.GetResultClusterCount()

		for i in range(i32ResultCount) :		
			flo = CFL3DObject()
			pointCloudClustering3D.GetResult3DObject(i, flo)

			tp3Color = TPoint3[Byte]()
			CColorPaletteUtilities.GetColor(i + 1, tp3Color.x, tp3Color.y, tp3Color.z)

			i32VertexCount = flo.GetVertices().Count
			listColors = List[TPoint3[Byte]]()
			
			for idx in range(i32VertexCount) :
		...
		# 3D 뷰가 종료될 때 까지 기다림 # Wait for 3D view to close
