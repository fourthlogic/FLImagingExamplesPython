...
def main():
	...
	while True:
		...
		if (res := fliOperandImage.Assign(fliSourceImage)).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		if (res := viewImageOpr.Create(550, 0, 1000, 480)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break

		# Destination 이미지 뷰 생성 // Create the destination image view
		if (res := viewImageDst.Create(1000, 0, 1450, 480)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		# 객체 생성 // Create object
		weightedGuidedFilter = CWeightedGuidedFilter()
