...
class CDeviceEventImageEx(CDeviceEventImageBase):
	...
	def OnAcquisition(self, deviceImage):
		while(True):
			...
			if(not isinstance(deviceImage, CDeviceCameraArena3D)):
				break
...
def main():
	...
	# Arena3D 카메라 선언 // Declare the Arena3D camera
	camArena3D = CDeviceCameraArena3D()
	...
	while True:
		...
		if(bAutoDetect):
			...
			er = camArena3D.GetAutoDetectCameraSerialNumbers(listSerialNumbers)
		...
		if(bAutoDetect):
			...
			if((er := camArena3D.AutoDetectCamera(i32SelectDevice)).IsFail()):
		...
		if((er := camArena3D.Initialize()).IsFail()):
