...
def main():
    ...
    for i in range(flfaContours.GetCount()):
        ...
        if (res := imgStatistics.GetMedian(mvMedian)[0]).IsFail():
            ErrorPrint(res, "Failed to get Median Value from the Blob object.")
            break

        strMedian = "Median : {:.3f}".format(mvMedian.GetAt(0))
        print("No. {} ".format(i) + strMedian)
        
        if (res := imgStatistics.GetLowerQuartile(mvLowerQuartile)[0]).IsFail():
            ErrorPrint(res, "Failed to get Lower Quartile Value from the Blob object.")
            break
        
        strLowerQuartile = "Lower Quartile : {:.3f}".format(mvLowerQuartile.GetAt(0))
        print("No. {} ".format(i) + strLowerQuartile)
        
        if (res := imgStatistics.GetUpperQuartile(mvUpperQuartile)[0]).IsFail():
            ErrorPrint(res, "Failed to get Upper Quartile Value from the Blob object.")
            break
