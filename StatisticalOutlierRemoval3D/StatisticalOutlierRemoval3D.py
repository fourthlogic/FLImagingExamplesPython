...
def main():
	...
	while True:		
		...
		if(res := floSourceObject.Load("../../ExampleImages/StatisticalOutlierRemoval3D/Cube_Noisy.fl3do")).IsFail() :		
		...
		# StatisticalOutlierRemoval3D 객체 생성 # Create StatisticalOutlierRemoval3D object
		statisticalOutlierRemoval3D = CStatisticalOutlierRemoval3D()

		# Destination object 설정 # Set the learn object
		statisticalOutlierRemoval3D.SetDestinationObject(floDestinationObject)
		...
		if(res := statisticalOutlierRemoval3D.Execute()).IsFail() :	
			ErrorPrint(res, "Failed to execute Statistical Outlier Removal 3D.")
			break
		...
		if(res := view3DDst.PushObject(statisticalOutlierRemoval3D.GetDestinationObject())).IsFail() :		
