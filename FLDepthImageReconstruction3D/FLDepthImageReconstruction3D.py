...
def main():
	...
	while True:
		...
		flDepthImageReconstruction3D = CFLDepthImageReconstruction3D()

		# Source 이미지 설정 # Set the source image
		if (res := flDepthImageReconstruction3D.SetSourceImage(fliSrcImage)[0]).IsFail():
			break
		# 결과 destination height map 이미지 설정 # Set the destination height map image
		if (res := flDepthImageReconstruction3D.SetDestinationHeightMapImage(fliDstImage)[0]).IsFail():
			break
		# 결과 destination texture 이미지 설정 # Set the destination texture image
		if (res := flDepthImageReconstruction3D.SetDestinationTextureImage(fliTxtImage)[0]).IsFail():
			break
		# Destination 3D object 설정 # Set the Destination 3D object 
		if (res := flDepthImageReconstruction3D.SetDestinationObject(floDstObject)[0]).IsFail():
			break
