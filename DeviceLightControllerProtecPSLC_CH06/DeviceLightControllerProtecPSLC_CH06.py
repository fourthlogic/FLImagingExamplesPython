...
def main():
	...
	lightController = CDeviceLightControllerProtecPSLC_CH06()
		...
		while True:
			...
			if i32OperationMode == 1:
				...
				if strInput.isdigit():
					...
					# 메모리 인덱스를 설정합니다. # Select the memory index.
					strInput = input("Select memory index: ")

					if strInput.isdigit():
						i32MemoryIndex = int(strInput)
						...
						if strInput.isdigit():
							...
							lightController.SetMutiShotStrob(i32TriggerIndex, i32MemoryIndex, i32ChannelIndex, UInt16(i32StrobeValue & 0xffff))
