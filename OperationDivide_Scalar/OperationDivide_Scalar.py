...
def main():
	...
	while True:
		...
		if (res := layersDestination[0].DrawTextCanvas(flpPoint, 'Destination1 Image(Divide 1.5)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')

		if (res := layersDestination[1].DrawTextCanvas(flpPoint, 'Destination2 Image(Divide 3.0)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
