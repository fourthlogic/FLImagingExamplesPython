...
def main():
	...
	while True:
		...
		#	- Sampling 메소드가 Min Gaussian, Mean Gaussian 모드인 경우 출력되는 인덱스 이미지는 각각 Min, Mean 모드로 동작할 때 출력되는 인덱스 이미지와 동일합니다. // If the sampling method is in Min Gaussian and Mean Gaussian modes, the output index image is the same as the output index image when operating in Min and Mean modes, respectively.
		...
		#	- Mean : 입력된 이미지 가운데 최대 값을 출력합니다. // Mean : Outputs the maximum value of the entered image.
		#	- MeanGaussian : 입력된 이미지 가운데 가장 앞 쪽 인덱스에 위치한 최대 값을 기준으로 가우시안 값을 출력합니다. // MeanGaussian : Outputs the Gaussian value based on the maximum value located in the leading index of the entered image.
