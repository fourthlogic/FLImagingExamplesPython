...
def main():
	...
	while True:
		...
		for i in range (0, int(EType.ETypeCount)):
			...
			if (res := (listViewImage[i].Create(x * 400 + 400, 0, x * 400 + 400 + 400,  400))).IsFail():
		...
		lowpassFilter = CLowpassFilter()
		# Source 이미지 설정 # Set the source image
		lowpassFilter.SetSourceImage(listFliImage[int(EType.Source)])
		# Source ROI 설정 # Set the Source ROI
		lowpassFilter.SetSourceROI(flrROI)

		# Destination1 이미지 설정 # Set the destination1 image
		lowpassFilter.SetDestinationImage(listFliImage[int(EType.Destination1)])
		...
		lowpassFilter.SetDestinationImage(listFliImage[int(EType.Destination2)])
		...
		if (res := (listLayer[int(EType.Destination1)].DrawTextCanvas(flpZero, "Destination1 Image (Lowpass1)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break

		if (res := (listLayer[int(EType.Destination2)].DrawTextCanvas(flpZero, "Destination2 Image (Lowpass2)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break
