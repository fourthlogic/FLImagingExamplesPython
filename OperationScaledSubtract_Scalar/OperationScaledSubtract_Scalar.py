...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationScaledSubtract/CityNight.flif')).IsFail():
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination1.DrawTextCanvas(flpPoint, 'Destination Image(ScaledSubtract 30, 0, 0)', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination2.DrawTextCanvas(flpPoint, 'Destination Image(ScaledSubtract 0, 0, 30)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
