...
def main():
	...
	while True:		
		...
		if(res := floSourceObject.Load("../../ExampleImages/PlaneEstimatorFromFeaturePoints3D/box.fl3do")).IsFail() :		
		...
		# PlaneEstimatorFromFeaturePoints3D 객체 생성 # Create PlaneEstimatorFromFeaturePoints3D object
		planeEstimatorFromFeaturePoints3D = CPlaneEstimatorFromFeaturePoints3D()
		# Destination object 설정 # Set the learn object
		planeEstimatorFromFeaturePoints3D.SetDestinationObject(floDestinationObject)
		# Source object 설정 # Set the source object
		planeEstimatorFromFeaturePoints3D.SetSourceObject(floSourceObject)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if(res := planeEstimatorFromFeaturePoints3D.Execute()).IsFail() :	
			ErrorPrint(res, "Failed to execute Plane Estimator From Feature Points 3D.")
			break
		...
		if(res := view3DDst.PushObject(planeEstimatorFromFeaturePoints3D.GetDestinationObject())).IsFail() or \
		   (res := view3DDst.PushObject(floSourceObject)).IsFail()	:
