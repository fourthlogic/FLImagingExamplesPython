...
def main():
	...
	while True:
		...
		if (res := viewImageSrc.Create(100, 0, 600, 500)).IsFail() or \
			(res := viewImageOpr.Create(600, 0, 1100, 500)).IsFail() or \
		...
		channelLpNorm = CChannelLpNorm()
		
		# Source 이미지 설정 # Set Source image
		if (res := channelLpNorm.SetSourceImage(fliSrcImage)[0]).IsFail():
			break

		# Operand 이미지 설정 # Set Operand image
		if (res := channelLpNorm.SetOperandImage(fliOprImage)[0]).IsFail():
			break

		# Destination 이미지 설정 # Set Destination image 
		if (res := channelLpNorm.SetDestinationImage(fliDstImage)[0]).IsFail():
			break
		...
		layerOpr = viewImageSrc.GetLayer(0)
