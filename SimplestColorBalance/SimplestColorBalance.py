...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/SimplestColorBalance/church.flif')).IsFail():
		...
		# Simplest Color Balance 객체 생성 # Create Simplest Color Balance object
		simplestColorBalance = CSimplestColorBalance()

		# Source 이미지 설정 # Set the source image
		simplestColorBalance.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 # Set the destination image
		simplestColorBalance.SetDestinationImage(fliDestination1Image)
		...
		# Destination 이미지 설정 # Set the destination image
		simplestColorBalance.SetDestinationImage(fliDestination2Image)
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination1.DrawTextCanvas(flpPoint, 'Destination1 Image (Color)', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination2.DrawTextCanvas(flpPoint, 'Destination2 Image (Intensity)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
