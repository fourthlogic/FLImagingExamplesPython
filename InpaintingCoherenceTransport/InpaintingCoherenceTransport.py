...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/InpaintingCoherenceTransport/owl.flif')).IsFail():
		...
		# Inpainting Coherence Transport 객체 생성 // Create Inpainting Coherence Transport object
		inpaintingCoherenceTransport = CInpaintingCoherenceTransport()

		# Source 이미지 설정 // Set the source image
		inpaintingCoherenceTransport.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 // Set the destination image
		inpaintingCoherenceTransport.SetDestinationImage(fliDestinationImage)
		...
		mvChannelWeight = CMultiVar[Double](0.114, 0.587, 0.299)
		...
		if (res := inpaintingCoherenceTransport.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Inpainting Coherence Transport.')
			break
		...
		if (res := fliSourceImage.PushBackFigure(CFigureUtilities.ConvertFigureObjectToString(flrInpaintingRegion))):
