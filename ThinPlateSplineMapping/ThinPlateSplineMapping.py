...
def main():
    ...
    while True:
        ...
        if (res := tps.Load("MappingData.fltps")).IsFail():
            ...
            for y in range(flpGridSize.y):
                ...
                for x in range(flpGridSize.x):
                    ...
                    flpDistortion = CFLPoint[Double](flpGridIndex.x + CRandomGenerator.Double(-0.15, 0.15), flpGridIndex.y + CRandomGenerator.Double(-0.15, 0.15))
        ...
        for k in range(tps.GetControlPointCount()):
            ...
            print(f"Source Vertex : ({flpSource.x:.3f},{flpSource.y:.3f})")
            print(f"Destination Vertex : ({flpDestination.x:.3f},{flpDestination.y:.3f})")
        ...
        for y in range(int((flpGridSize.y - 1) * f64Slice) + 1):
            flpdSource.y = (y / f64Slice)

            for x in range(int((flpGridSize.x - 1) * f64Slice) + 1):
                flpdSource.x = (x / f64Slice)
                flpaSource.PushBack(flpdSource) # PointArray에 추가 // Add to PointArray
