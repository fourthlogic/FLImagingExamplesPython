...
def main():
	...
	while True:
		...
		# 객체 생성 # Create object
		operationLogicalAnd = COperationLogicalAnd()

		# Source 이미지 설정 # Set the source image
		operationLogicalAnd.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 # Set the destination image
		operationLogicalAnd.SetDestinationImage(fliDestination1Image)

		# ROI 범위 설정 # Set the ROI value
		flcSourceROI = CFLCircle[Double](128, 128, 80, 0, 0, 360, ERadialShapeType.Segment)
		...
		if (res := layerDestination1.DrawFigureImage(flcSourceROI, EColor.LIME)).IsFail():
			ErrorPrint(res, 'Failed to draw figure.')
			break
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination1.DrawTextCanvas(flpPoint, 'Source & Destination Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() :
		...
		while viewImageSrc.IsAvailable() and viewImageDst1.IsAvailable() :
