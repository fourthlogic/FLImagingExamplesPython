...
def main():
    ...
    lightControllerNARATECH_NRT_PW = CDeviceLightControllerNARATECH_NRT_PW()
    ...
    while True:  # do ~ while 구조 // do-while loop structure
        ...
        if portNumber.isdigit():
            ...
            lightControllerNARATECH_NRT_PW.SetComPortNumber(int(portNumber))
        ...
        if baudRate.isdigit():
            ...
            lightControllerNARATECH_NRT_PW.SetBaudRate(int(baudRate))

        if (res := lightControllerNARATECH_NRT_PW.Initialize()).IsFail():
            ...
            if channel.isdigit():
                ...
                if operationMode == 1:
                    ...
                    if onOff.isdigit():
                        ...
                        lightControllerNARATECH_NRT_PW.SetChannelState(channel, int(onOff) == 1)

                elif operationMode == 2:
                    lightValue = input("Input Light Value (0 ~ 255): ")
                    if lightValue.isdigit():
                        # 조명 값을 설정합니다. // Set the light value.
                        lightControllerNARATECH_NRT_PW.SetLightValue(channel, int(lightValue))
    ...
    if (res := lightControllerNARATECH_NRT_PW.Terminate()).IsFail():
