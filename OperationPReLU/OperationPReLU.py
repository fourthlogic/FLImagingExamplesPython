...
def main():
	...
	while True:
		...
		operationPReLU = COperationPReLU()
		
		if (res := operationPReLU.SetSourceImage(fliSrcImage)[0]).IsFail():
			break
		if (res := operationPReLU.SetDestinationImage(fliDstImage0)[0]).IsFail():
			break
		...
		if (res := operationPReLU.SetDestinationImage(fliDstImage1)[0]).IsFail():
			break
