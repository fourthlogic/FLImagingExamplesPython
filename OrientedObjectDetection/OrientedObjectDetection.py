...
def main():
	...
	while True:
		...
		orientedObjectDetectionDL.SetLearningImage(fliLearnImage)
		# 검증할 이미지 설정 # Set the image to validation
		orientedObjectDetectionDL.SetLearningValidationImage(fliValidationImage)
		# 분류할 이미지 설정 # Set the image to classify
		orientedObjectDetectionDL.SetInferenceImage(fliValidationImage)
		...
		def Learn_thread():
			...
			eLearnResult = orientedObjectDetectionDL.Learn()
			bTerminated = True
		...
		while not orientedObjectDetectionDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = orientedObjectDetectionDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			i32MiniBatchCount = orientedObjectDetectionDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = orientedObjectDetectionDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = orientedObjectDetectionDL.GetLastEpoch()
			
			# 미니 배치 반복이 완료되면 cost와 validation 값을 디스플레이 
			# Display cost and validation value if iterations of the mini batch is completed 
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				# 마지막 학습 결과 비용 받기 # Get the last cost of the learning result
				f32CurrCost = orientedObjectDetectionDL.GetLearningResultLastCost()
				# 마지막 평균 학습 결과 비용 받기 # Get the last cost of the learning result
				f32AvgCost = orientedObjectDetectionDL.GetLearningResultLastAverageCost()
				...
				orientedObjectDetectionDL.GetLearningResultAllHistory(listCostHistory, listAvgCostHistory, listMeanAPHistory, vctValidationEpoch, listRecallHistory , listPrecisionHistory)
				...
				if((listCostHistory.Count != 0 and i32PrevCostCount != listCostHistory.Count) or (listMeanAPHistory.Count != 0 and i32PrevValidationCount != listMeanAPHistory.Count)):
					i32Step = orientedObjectDetectionDL.GetLearningValidationStep()
					listX = List[Single]()
			...
			if(orientedObjectDetectionDL.IsRunning() == False):
				break
