...
def main():
	...
	while True:
		...
		fasterCharacterBasedOCRDL.SetLearningValidationImage(fliValidationImage)
		...
		def Learn_thread():
			...
			eLearnResult = fasterCharacterBasedOCRDL.Learn()
			bTerminated = True
		...
		while not fasterCharacterBasedOCRDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = fasterCharacterBasedOCRDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			i32MiniBatchCount = fasterCharacterBasedOCRDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = fasterCharacterBasedOCRDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = fasterCharacterBasedOCRDL.GetLastEpoch()
			
			# 미니 배치 반복이 완료되면 cost와 validation 값을 디스플레이 
			# Display cost and validation value if iterations of the mini batch is completed 
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				# 마지막 학습 결과 비용 받기 # Get the last cost of the learning result
				f32CurrCost = fasterCharacterBasedOCRDL.GetLearningResultLastCost()
				# 마지막 검증 결과 받기 # Get the last validation result
				f32ValidationPa = fasterCharacterBasedOCRDL.GetLearningResultLastAccuracy()
				f32ValidationPaMeanIoU = fasterCharacterBasedOCRDL.GetLearningResultLastMeanIoU()
				...
				fasterCharacterBasedOCRDL.GetLearningResultAllHistory(listCostHistory, listValidationHistory, listMeanIoUHistory, listValidationsZEHistory, listMeanIoUZEHistory, listMeanAPHistory, vctValidationEpoch)
			...
			if(fasterCharacterBasedOCRDL.IsRunning() == False):
				break
