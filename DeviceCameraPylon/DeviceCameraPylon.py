...
def main():
	...
	# Pylon 카메라 선언 # Declare the Pylon camera
	camPylon = CDeviceCameraPylon()
	...
	while True:
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camPylon.GetAutoDetectGigECameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.IEEE1394):
				er = camPylon.GetAutoDetectIEEE1394CameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camPylon.GetAutoDetectUSBCameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.CameraLink):
				er = camPylon.GetAutoDetectCameraLinkCameraSerialNumbers(listSerialNumbers)
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camPylon.AutoDetectGigECamera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.IEEE1394):
				er = camPylon.AutoDetectIEEE1394Camera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camPylon.AutoDetectUSBCamera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.CameraLink):
				er = camPylon.AutoDetectCameraLinkCamera(i32SelectDevice)
		...
		if((er := camPylon.Initialize()).IsFail()):
