...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/Stitching/Mountain.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Source 이미지 뷰 생성 // Create source image view
		if (res := viewImageSrc.Create(300, 0, 820, 430)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break

		# Destination 이미지 뷰 생성 // Create destination image view
		if (res := viewImageDst.Create(820, 0, 1430, 430)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
