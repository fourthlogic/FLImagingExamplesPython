...
def main():
	...
	while True:
		...
		if (res := fliLearnImage.Load('../../ExampleImages/ShadingCalibrator/Learn.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Source 이미지 로드 // Load the source image
		if (res := fliSourceImage.Load('../../ExampleImages/ShadingCalibrator/Src.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		if (res := viewImageDst.Create(1124, 0, 1736, 512)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		# 두 이미지 뷰의 시점을 동기화 한다 // Synchronize the viewpoints of the two image views
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. // A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageSrc.SynchronizePointOfView(viewImageLearn)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize view.')
			break
		...
		# 두 이미지 뷰 윈도우의 위치를 맞춤 // Synchronize the positions of the two image view windows
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. // A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageSrc.SynchronizeWindow(viewImageLearn)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize window.')
			break

		# ShadingCalibrator 객체 생성 // Create ShadingCalibrator object
		shadingCalibrator = CShadingCalibrator()
		...
		if (res := shadingCalibrator.Calibrate()).IsFail():
			ErrorPrint(res, 'Failed to calibrate the image.')
			break
		...
		if (res := layerLearn.DrawTextCanvas(flpPoint, 'Learn Image', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
			(res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
		...
		while viewImageLearn.IsAvailable() and viewImageSrc.IsAvailable() and viewImageDst.IsAvailable():
