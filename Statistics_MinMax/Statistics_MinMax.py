...
def main():
	...
	while True:
		...
		if (res := statistics.GetMin(mvMin)[0]).IsFail():
		...
		if (res := statistics.GetMax(mvMax)[0]).IsFail():
			ErrorPrint(res, 'Failed to process.')
			break
		...
		# 이미지 전체(혹은 ROI 영역) 픽셀값의 최소값을 구하는 함수 // Function that calculate the min of the pixel value of the image(or the region of ROI)
		if (res := statistics.GetMin(mvTrimmingMin)[0]).IsFail():
			ErrorPrint(res, 'Failed to process.')
			break

		# 이미지 전체(혹은 ROI 영역) 픽셀값의 최대값을 구하는 함수 // Function that calculate the max of the pixel value of the image(or the region of ROI)
		if (res := statistics.GetMax(mvTrimmingMax)[0]).IsFail():
			ErrorPrint(res, 'Failed to process.')
			break
		...
		strMinValue = 'Min Of Region : {}'.format(mvMin)
		strMaxValue = 'Max Of Region : {}'.format(mvMax)

		strTrimmingMinValue = 'Min Of Trimmed Region : {}'.format(mvTrimmingMin)
		strTrimmingMaxValue = 'Max Of Trimmed Region : {}'.format(mvTrimmingMax)
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, strMinValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail():
		...
		# 이미지 뷰 정보 표시 // Display image view information
		if (res := layerSource.DrawTextCanvas(flpPoint, strMaxValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		...
		# 이미지 뷰 정보 표시 // Display image view information
		if (res := layerSource.DrawTextCanvas(flpPoint, strTrimmingMinValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		...
		# 이미지 뷰 정보 표시 // Display image view information
		if (res := layerSource.DrawTextCanvas(flpPoint, strTrimmingMaxValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		flrMinRegion = CFLRect[Double](429, 336, 430, 337)
		flrMaxRegion = CFLRect[Double](492, 370, 493, 371)
		...
		if (res := layerSource.DrawFigureImage(flrMinRegion, EColor.BLUE, 1, EColor.LIGHTBLUE, EGUIViewImagePenStyle.Solid, 0.5, 0.5)).IsFail():
			ErrorPrint(res, 'Failed to draw figure.')
			break

		if (res := layerSource.DrawFigureImage(flrMaxRegion, EColor.LIGHTRED, 1, EColor.RED, EGUIViewImagePenStyle.Solid, 0.5, 0.5)).IsFail():
			ErrorPrint(res, 'Failed to draw figure.')
			break
