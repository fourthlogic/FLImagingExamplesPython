...
def main():
	...
	while True:
		...
		res = fliSourceImage.Load('../../ExampleImages/OperationScaledDivide/Generator.flif')
		...
		res = viewImageDst1.Create(612, 0, 1124, 512)
		...
		res = viewImageDst2.Create(1124, 0, 1636, 512)
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		res = viewImageDst1.SetImagePtr(fliDestination1Image)
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		res = viewImageDst2.SetImagePtr(fliDestination2Image)
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		res = viewImageSrc.SynchronizePointOfView(viewImageDst1)
		...
		# 두 이미지 뷰의 시점을 동기화 한다 # Synchronize the viewpoints of the two image views
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		res = viewImageSrc.SynchronizePointOfView(viewImageDst2)
		...
		res = layerDestination1.DrawTextCanvas(flpPoint, 'Destination Image(ScaledDivide 192)', EColor.YELLOW, EColor.BLACK, 18)

		if res.IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		
		res = layerDestination2.DrawTextCanvas(flpPoint, 'Destination Image(ScaledDivide 512)', EColor.YELLOW, EColor.BLACK, 18)
