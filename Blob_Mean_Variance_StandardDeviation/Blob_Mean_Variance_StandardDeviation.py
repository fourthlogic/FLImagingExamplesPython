...
def main():
    ...
    for i in range(flfaContours.GetCount()):
        ...
        if (res := imgStatistics.GetMean(mvMean)[0]).IsFail():
            ErrorPrint(res, "Failed to get Mean Value from the Blob object.")
            break

        strMean = "Mean : {:.3f}".format(mvMean.GetAt(0))
        print("No. {} ".format(i) + strMean)
        
        if (res := imgStatistics.GetVariance(mvVariance)[0]).IsFail():
            ErrorPrint(res, "Failed to get Variance Value from the Blob object.")
            break
        
        strVariance = "Variance : {:.3f}".format(mvVariance.GetAt(0))
        print("No. {} ".format(i) + strVariance)
        
        if (res := imgStatistics.GetStandardDeviation(mvStandardDeviation)[0]).IsFail():
