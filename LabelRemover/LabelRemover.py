...
def main():
	...
	while True:
		...
		if (res := viewImageDst.Create(600, 0, 1100, 500)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		# 두 이미지 뷰의 시점을 동기화 한다 # Synchronize the viewpoints of the two image views
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageSrc.SynchronizePointOfView(viewImagesDstRegex)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize view.')
			break
		...
		# Destination 이미지 뷰에 이미지를 디스플레이 # Display the image in the destination image view
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImagesDstRegex.SetImagePtr(fliResultRegexImage)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break
		...
		# 이미지 뷰 윈도우의 위치를 맞춤 # Synchronize the positions of the image view windows
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageSrc.SynchronizeWindow(viewImagesDstRegex)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize window.')
			break


		# CLabelRemoverDL 객체 생성 # Create CLabelRemoverDL object
		labelRemoverDL = CLabelRemoverDL()

		# Source 이미지 설정 # Set the source image
		labelRemoverDL.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 # Set the destination image
		labelRemoverDL.SetDestinationImage(fliDestinationImage)
		...
		layerDestinationRegex = viewImagesDstRegex.GetLayer(0)

		# 기존에 Layer에 그려진 도형들을 삭제 # Clear the figures drawn on the existing layer
		layerSource.Clear()
		layerDestination.Clear()
		layerDestinationRegex.Clear()
		...
		viewImagesDstRegex.ZoomFit()
		viewImageSrc.RedrawWindow()
		viewImageDst.RedrawWindow()
		viewImagesDstRegex.ZoomFit()

		# # 이미지 뷰가 닫히기 전까지 종료하지 않고 대기 # Wait until the image view is closed before exiting
		while viewImageSrc.IsAvailable() and viewImageDst.IsAvailable() and viewImagesDstRegex.IsAvailable():
			CThreadUtilities.Sleep(1)

		viewImageSrc.Destroy()
		viewImageDst.Destroy()
		viewImagesDstRegex.Destroy()
