...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/Histogram/Flower.flif')).IsFail():
		...
		normalization = CNormalization()

		# Source 이미지 설정 // Set the source image
		normalization.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 // Set the destination image
		normalization.SetDestinationImage(fliDestinationImage)

		# 정규화 방식 설정(Max Element) // Set the Normalization Method(Max Element)
		normalization.SetNormalizationMethod(CNormalization.ENormalizationMethod.MaxElement)

		# 정규화 범위 설정 // Set the normalization range
		# Min 범위 설정 // Set the Min Range
		normalization.SetMinRange(30)

		# Max 범위 설정 // Set the Max Range
