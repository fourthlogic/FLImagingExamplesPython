...
def main():
	...
	while True:
		...
		# Operand 이미지를 Source 이미지와 동일한 이미지로 생성 # Create operand image as same as source image
		if (res := fliOperandImage.Load('../../ExampleImages/OperationMatrixMultiply/Matrix1.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# 두 이미지 뷰의 시점을 동기화 한다 # Synchronize the viewpoints of the two image views
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageOpr.SynchronizePointOfView(viewImageDst)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize view.')
			break
		...
		# 두 이미지 뷰 윈도우의 위치를 맞춤 # Synchronize the positions of the two image view windows
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageOpr.SynchronizeWindow(viewImageDst)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize window.')
			break
		...
		operationMatrixMultiply.SetOperandImage(fliOperandImage)

		# Destination 이미지 설정 # Set the destination image
		operationMatrixMultiply.SetDestinationImage(fliDestinationImage)
