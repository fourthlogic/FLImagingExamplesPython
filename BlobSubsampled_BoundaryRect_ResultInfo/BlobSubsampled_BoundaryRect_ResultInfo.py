...
def main():
    ...
    for i in range(flfaBoundaryRects.GetCount()):
        ...
        if flrRect is not None:
            ...
            strRightBottom = "(RB :{:.2f}, {:.2f})".format(flrRect.right, flrRect.bottom)
            strInfo = "Width : {:.2f}\nHeight : {:.2f}\nCenter : ({:.2f}, {:.2f})".format(flrRect.GetWidth(), flrRect.GetHeight(), flrRect.GetCenter().x, flrRect.GetCenter().y)
