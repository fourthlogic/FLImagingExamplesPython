...
def main():
    ...
    while True:
        ...
        for y in range(lczm.GetRow()):
            for x in range(lczm.GetColumn()):
                vertex = lczm.GetControlPoint(CFLPoint[Int32](x, y))
        ...
        for y in range(int((lczm.GetRow() - 1) * f64Slice) + 1):
            flpdSource.y = y / f64Slice

            for x in range(int((lczm.GetColumn() - 1) * f64Slice) + 1):
        ...
        if (resConvertSrcToDst := lczm.ConvertSourceToDestination(flpaSource, flpaDestination))[0].IsOK():
        ...
        if (resConvertDstToSrc := lczm.ConvertDestinationToSource(flpaDestination, flpaConvertedSource))[0].IsOK():
