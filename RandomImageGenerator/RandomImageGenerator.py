...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/RandomImageGenerator/Figure.flif')).IsFail():
		...
		randomImageGenerator.SetSourceImage(fliDestinationImage)

		# ROI 지정 // Set the source ROI
		flfaSrcROI = CFLFigureArray()
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpPoint, 'Random Image Generator Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
