...
def main():
    ...
    for i in range(i32ExampleCount):
        ...
        if (res := arrViewImage[i].Create(i32X, i32Y, i32X + 450, i32Y + 450)).IsFail():
    ...
    for i in range(i32ExampleCount):        
        ...
        for j in range(flfaResultCorners.GetCount()):
            print(
                "#{} : Corner X : {}, Corner Y : {}".format(
                    j,
                    flfaResultCorners.GetAt(j).GetCenter().x,
                    flfaResultCorners.GetAt(j).GetCenter().y,
                )
            )
