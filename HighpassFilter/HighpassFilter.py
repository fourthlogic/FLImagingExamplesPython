...
def main():
	...
	while True:
		...
		# 이미지 로드 # Load image
		if (res := listFliImage[int(EType.Source)].Load("../../ExampleImages/Filter/Binding.flif")).IsFail():
		...
		highpassFilter = CHighpassFilter()
		# Source 이미지 설정 # Set the source image
		highpassFilter.SetSourceImage(listFliImage[int(EType.Source)])
		# Source ROI 설정 # Set the Source ROI
		highpassFilter.SetSourceROI(flrROI)

		# Destination1 이미지 설정 # Set the destination1 image
		highpassFilter.SetDestinationImage(listFliImage[int(EType.Destination1)])
		...
		highpassFilter.SetDestinationImage(listFliImage[int(EType.Destination2)])
		...
		if (res := (listLayer[int(EType.Destination1)].DrawTextCanvas(flpZero, "Destination1 Image (Highpass1)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break

		if (res := (listLayer[int(EType.Destination2)].DrawTextCanvas(flpZero, "Destination2 Image (Highpass2)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break
