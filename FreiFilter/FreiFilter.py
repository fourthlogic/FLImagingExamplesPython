...
def main():
	...
	while True:
		...
		res = arrFliImage[0].Load("../../ExampleImages/EdgeDetection/Alphabat.flif")
		if res.IsFail():
		...
		# Frei Filter 객체 생성 // Create Frei Filter object
		freiFilter = CFreiFilter()

		# Source 이미지 설정 // Set the source image
		freiFilter.SetSourceImage(arrFliImage[0])
		# Destination 이미지 설정 // Set the destination image
		freiFilter.SetDestinationImage(arrFliImage[1])

		# 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if (res := freiFilter.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Frei Filter.')
			break
