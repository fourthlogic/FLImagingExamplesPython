...
def main():
	...
	while True:
		...
		if (res := viewImageSrc.Create(112, 0, 912, 534)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break

		# Destination 이미지 뷰 생성 # Create the destination image view
		if (res := viewImageDst.Create(913, 0, 1713, 534)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		colorConverter = CColorConverter()
		...
		# 변환할 픽셀 포맷 입력(C3_U8 -> C4_U8) # Set the pixel format to convert(C3_U8 -> C4_U8)
		colorConverter.SetPixelFormat(EPixelFormat.C4_U8)

		# 변환할 컬러 시퀀스 입력(BGR -> ARGB) # Set the color sequence to convert(BGR -> ARGB)
		colorConverter.SetColorSequence(EColorSequence.ARGB)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if (res := colorConverter.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Color Converter.')
			break
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image (BGR)', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpPoint, 'Destination Image (ARGB)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
