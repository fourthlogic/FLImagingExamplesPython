...
def main():
	...
	fliResultImageOK = CFLImage()
	fliResultImageDamaged = CFLImage()

	# 이미지 뷰 선언 # Declare the image view
	viewImageLearn = CGUIViewImage()
	viewImageValidate = CGUIViewImage()
	viewImageResultOK = CGUIViewImage()
	viewImageResultDamaged = CGUIViewImage()
	...
	while True:
		...
		if (res := fliValidateImage.Load('../../ExampleImages/GenerativeAdversarialNetwork/CircleLabel_Validation.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# Result 이미지 뷰 생성 # Create source image view
		if (res := viewImageResultDamaged.Create(612, 512, 1124, 1024)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageResultOK.SetImagePtr(fliResultImageOK)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break

		# Result 이미지 뷰에 이미지를 디스플레이 # Display the image in the source image view
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewImageResultDamaged.SetImagePtr(fliResultImageDamaged)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break
		...
		layerResultOK = viewImageResultOK.GetLayer(0)
		layerResultDamaged = viewImageResultDamaged.GetLayer(0)

		# 기존에 Layer에 그려진 도형들을 삭제 # Clear the figures drawn on the existing layer
		layerLearn.Clear()
		layerValidate.Clear()
		layerResultOK.Clear()
		layerResultDamaged.Clear()
		...
		if (res := layerResultDamaged.DrawTextCanvas(flpPoint, 'Damaged', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		...
		viewImageResultDamaged.ZoomFit()

		# 이미지 뷰를 갱신 # Update image view
		viewImageLearn.Invalidate(True)
		viewImageValidate.Invalidate(True)
		viewImageResultOK.Invalidate(True)
		viewImageResultDamaged.Invalidate(True)

		# 그래프 뷰를 갱신 # Update the Graph view.
		viewGraph.Invalidate(True)

		# 이미지 뷰가 종료될 때 까지 기다림 # Wait for the image view to close
		while viewImageLearn.IsAvailable() and viewImageResultOK.IsAvailable() and viewImageResultDamaged.IsAvailable() and viewImageValidate.IsAvailable() and viewGraph.IsAvailable():
			CThreadUtilities.Sleep(1)

		viewImageLearn.Destroy()
		viewImageResultOK.Destroy()
		viewImageResultDamaged.Destroy()
