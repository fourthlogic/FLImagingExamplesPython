...
def main():
	...
	while True:
		...
		measurementInstanceDL = CMeasurementInstanceDL()

		# 학습할 이미지 설정 # Set the image to learn
		measurementInstanceDL.SetLearningImage(fliLearnImage)
		# 검증할 이미지 설정 # Set the image to validation
		measurementInstanceDL.SetLearningValidationImage(fliValidationImage)
		# 분류할 이미지 설정 # Set the image to classify
		measurementInstanceDL.SetInferenceImage(fliValidationImage)
		measurementInstanceDL.SetInferenceResultImage(fliResultImage)
		...
		# 설정한 Optimizer를 measurementInstanceDL에 적용 # Apply Optimizer that we set up to measurementInstanceDL
		...
		# 자동 저장 조건식 설정. map값이 최대 값인 경우 저장 활성화
		# Set auto-save conditional expressions. Enable save if the map value is maximum
		...
		def Learn_thread():
			...
			eLearnResult = measurementInstanceDL.Learn()
			bTerminated = True
		...
		while not measurementInstanceDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = measurementInstanceDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			i32MiniBatchCount = measurementInstanceDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = measurementInstanceDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = measurementInstanceDL.GetLastEpoch()
			
			# 미니 배치 반복이 완료되면 cost와 validation 값을 디스플레이 
			# Display cost and validation value if iterations of the mini batch is completed 
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				# 마지막 학습 결과 비용 받기 # Get the last cost of the learning result
				f32CurrCost = measurementInstanceDL.GetLearningResultLastCost()
				# 마지막 검증 결과 받기 # Get the last validation result
				f32MeanAP = measurementInstanceDL.GetLearningResultLastMeanAP()
				# 마지막 Recall 결과 받기 # Get the last recall result
				f32Recall = measurementInstanceDL.GetLearningResultLastRecall()
				# 마지막 검증 결과 받기 # Get the last validation result
				f32Precision = measurementInstanceDL.GetLearningResultLastPrecision()
				...
				measurementInstanceDL.GetLearningResultAllHistory(listCostHistory, listMeanAPHistory, listValidationEpoch, listRecall, listPrecision)
				...
				if((listCostHistory.Count != 0 and i32PrevCostCount != listCostHistory.Count) or (listMeanAPHistory.Count != 0 and i32PrevValidationCount != listMeanAPHistory.Count)):
					i32Step = measurementInstanceDL.GetLearningValidationStep()
					listX = List[Single]()
			...
			if(measurementInstanceDL.IsRunning() == False):
				break
