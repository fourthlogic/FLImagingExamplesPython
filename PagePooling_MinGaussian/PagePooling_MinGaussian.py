...
def main():
	...
	while True:
		...
		#	- Sampling 메소드가 MinGaussian Gaussian, MinGaussian Gaussian 모드인 경우 출력되는 인덱스 이미지는 각각 MinGaussian, MinGaussian 모드로 동작할 때 출력되는 인덱스 이미지와 동일합니다. # If the sampling method is in MinGaussian Gaussian and MinGaussian Gaussian modes, the output index image is the same as the output index image when operating in MinGaussian and MinGaussian modes, respectively.
		...
		#	- MinGaussian : 입력된 이미지 가운데 최대 값을 출력합니다. # MinGaussian : Outputs the maximum value of the entered image.
		#	- MinGaussianGaussian : 입력된 이미지 가운데 가장 앞 쪽 인덱스에 위치한 최대 값을 기준으로 가우시안 값을 출력합니다. # MinGaussianGaussian : Outputs the Gaussian value based on the maximum value located in the leading index of the entered image.
		#	- MinGaussian : 입력된 이미지 가운데 최소 값을 출력합니다. # MinGaussian : Outputs the minimum value of the entered image.
		#	- MinGaussianGaussian : 입력된 이미지 가운데 가장 앞 쪽 인덱스에 위치한 최소 값을 기준으로 가우시안 값을 출력합니다. # MinGaussianGaussian : Outputs the Gaussian value based on the minimum value located in the leading index of the entered image.
