...
def main():
	...
	while True:
		...
		# 이미지 로드 // Load image
		if (res := listFliImage[int(EType.Source)].Load("../../ExampleImages/NoiseImage/NoiseImage1.flif")).IsFail():
		...
		uniformFilter = CUniformFilter()
		# Source 이미지 설정 // Set the source image
		uniformFilter.SetSourceImage(listFliImage[int(EType.Source)])
		# Source ROI 설정 // Set the Source ROI
		uniformFilter.SetSourceROI(flrROI)

		# Destination1 이미지 설정 // Set the destination1 image
		uniformFilter.SetDestinationImage(listFliImage[int(EType.Destination1)])
		...
		uniformFilter.SetDestinationImage(listFliImage[int(EType.Destination2)])
		...
		uniformFilter.SetDestinationImage(listFliImage[int(EType.Destination3)])
		...
		uniformFilter.SetDestinationImage(listFliImage[int(EType.Destination4)])
		...
		if (res := (listLayer[int(EType.Destination1)].DrawTextCanvas(flpZero, "Destination1 Image (Kernel Size 3x3)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break

		if (res := (listLayer[int(EType.Destination2)].DrawTextCanvas(flpZero, "Destination2 Image (Kernel Size 5x5)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break

		if (res := (listLayer[int(EType.Destination3)].DrawTextCanvas(flpZero, "Destination3 Image (Kernel Size 7x7)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break

		if (res := (listLayer[int(EType.Destination4)].DrawTextCanvas(flpZero, "Destination4 Image (Kernel Size 15x15)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break
