...
def main():
	...
	while True:
		...
		if (res := arrFliImage[EType.Source].Load('../../ExampleImages/Illuminate/Basement.flif')).IsFail():
		...
		# 객체 생성 # Create object
		illuminateFilter = CIlluminateFilter()

		# Source 이미지 설정 # Set the source image
		illuminateFilter.SetSourceImage(arrFliImage[EType.Source])

		# Destination 이미지 설정 # Set the destination image
		illuminateFilter.SetDestinationImage(arrFliImage[EType.Destination1])

		# Kernel Size = 41 설정 # Set the Kernel Size = 41
		...
		illuminateFilter.SetDestinationImage(arrFliImage[EType.Destination2])

		# Kernel Size = 151 설정 # Set the Kernel Size = 151
		...
		if (res := arrLayer[EType.Destination1].DrawTextCanvas(flpPoint, 'IlluminateFilter Size : 41x41 Factor: 0.55', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination2].DrawTextCanvas(flpPoint, 'IlluminateFilter Size : 151x151 Factor: 0.8', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
