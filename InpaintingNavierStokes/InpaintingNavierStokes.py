...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/InpaintingNavierStokes/Dandelion.flif')).IsFail():
		...
		# Inpainting Navier Stokes 객체 생성 // Create Inpainting Navier Stokes object
		inpaintingNavierStokes = CInpaintingNavierStokes()

		# Source 이미지 설정 // Set the source image
		inpaintingNavierStokes.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 // Set the destination image
		inpaintingNavierStokes.SetDestinationImage(fliDestinationImage)
		...
		if (res := inpaintingNavierStokes.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Inpainting Navier Stokes.')
			break
		...
		if (res := fliSourceImage.PushBackFigure(CFigureUtilities.ConvertFigureObjectToString(flfaInpaintingRegion))):
