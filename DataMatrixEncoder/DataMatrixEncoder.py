...
def main():
    ...
    dataMatrixEncoder = CDataMatrixEncoder()
    codeSpec = CDataMatrixSpec()
    ...
    # Encoding DataMatrix 타입 설정 # Set the DataMatrix encoding type.
    codeSpec.SetColorMode(EDataCodeColor.WhiteOnBlack)
    
    # DataMatrix 스펙 설정 # Set DataMatrix specification
    dataMatrixEncoder.SetDataMatrixEncodingSpec(codeSpec)

    # Encoding Message 설정 # Set encoding message
    ...
    for i in range(i64Results):
        # DataMatrix Decoder 결과를 얻어오기 위해 FLQuadD 선언 # Declare FLQuadD to retrieve the result from the DataMatrix Decoder.
        flqRegion = CFLQuad[Double]()

        # DataMatrix Decoder 결과들 중 Data Region 을 얻어옴 # Gets the Data Region from the DataMatrix Decoder results.
        if (res := dataMatrixDecoder.GetResultDataRegion(i, flqRegion)[0]).IsFail():
        ...
        if (res := dataMatrixDecoder.GetResultGridRegion(i, flfaGridRegion)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
        
        strDecodedMsg = StringBuilder()
        # DataMatrix Decoder 결과들 중 Decoded String 을 얻어옴 # Gets the decoded string from the results of the DataMatrix Decoder.
        if (res := dataMatrixDecoder.GetResultDecodedString(i, strDecodedMsg)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
