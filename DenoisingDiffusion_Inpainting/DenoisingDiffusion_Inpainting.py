...
def main():
	...
	while True:
		...
		if (res := fliValidateImage.Load('../../ExampleImages/DenoisingDiffusion/Ball_Validation.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# 객체 생성 # Create object
		denoisingDiffusionInpaintingDL = CDenoisingDiffusionInpaintingDL()

		# 학습할 이미지 설정 # Set the image to learn
		denoisingDiffusionInpaintingDL.SetLearningImage(fliLearnImage)
		# 검증할 이미지 설정 # Set the image to validate
		denoisingDiffusionInpaintingDL.SetLearningValidationImage(fliValidateImage)
		...
		# 설정한 Optimizer를 DenoisingDiffusionInpaintingDL에 적용 # Apply Optimizer that we set up to DenoisingDiffusionInpaintingDL
		...
		while not denoisingDiffusionInpaintingDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = denoisingDiffusionInpaintingDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while True:
			...
			i32MaxIteration = denoisingDiffusionInpaintingDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = denoisingDiffusionInpaintingDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = denoisingDiffusionInpaintingDL.GetLastEpoch()
			...
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MaxIteration and i32Epoch > 0:
				...
				res = denoisingDiffusionInpaintingDL.GetLearningResultAllHistory(listCosts, listSSIM, listMRQ, listValidationEpoch)[0]
				...
				if listCosts.Count != 0:
					...
					if (listCosts.Count != 0 and i32PrevCostCount != listCosts.Count) or (listSSIM.Count != 0 and i32PrevValidationCount != listSSIM.Count):
						...
						i32Step = denoisingDiffusionInpaintingDL.GetLearningValidationStep()
						listV1 = List[Single]()
						...
						for i in range(listMRQ.Count - 1):
			...
			if denoisingDiffusionInpaintingDL.IsRunning() == False:
				break
		...
		denoisingDiffusionInpaintingDL.SetInferenceImage(fliSourceImage)

		# 생성할 이미지 설정 # Set the image to create
		denoisingDiffusionInpaintingDL.SetInferenceResultImage(fliResultImage)
