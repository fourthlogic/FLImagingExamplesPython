...
def main():
	...
	while True:
		...
		if (res := fliOperandImage.Load('../../ExampleImages/OperationBlend/Flower.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		if (res := layerDestination.DrawTextCanvas(flpPointSub, 'Source Ratio 0.75, Operand Ratio 0.25', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
