...
def main():
	...
	while True:
		...
		# Kirsch Filter 객체 생성 // Create Kirsch Filter object
		kirschFilter = CKirschFilter()

		# Source 이미지 설정 // Set the source image
		kirschFilter.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 // Set the destination image
		kirschFilter.SetDestinationImage(fliDestinationImage)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if (res := kirschFilter.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute KirschFilter.')
			break
