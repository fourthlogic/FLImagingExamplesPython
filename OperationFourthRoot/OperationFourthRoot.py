...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationFourthRoot/Sea3ChF32.flif')).IsFail():
		...
		# Fourth Root 객체 생성 # Create Fourth Root object
		operationFourthRoot = COperationFourthRoot()

		# Source 이미지 설정 # Set the source image
		operationFourthRoot.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 # Set the destination image
		operationFourthRoot.SetDestinationImage(fliDestinationImage)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if (res := operationFourthRoot.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Fourth Root.')
			break
