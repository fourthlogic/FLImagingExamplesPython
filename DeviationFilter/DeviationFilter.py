...
def main():
	...
	while True:
		...
		if (res := arrFliImage[EType.Source].Load('../../ExampleImages/EdgeDetection/Alphabat.flif')).IsFail():
		...
		if (res := arrLayer[EType.Destination1].DrawTextCanvas(flpPoint, 'Filter Size : 5x5', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination2].DrawTextCanvas(flpPoint, 'Filter Size : 25x25', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
