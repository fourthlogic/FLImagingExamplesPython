...
def main():
	...
	while True:
		...
		if (res := viewImageSrc.Create(100, 0, 600, 545)).IsFail() or \
			(res := viewImageOpr.Create(600, 0, 1100, 545)).IsFail() or \
			(res := viewImageDst.Create(1100, 0, 1600, 545)).IsFail():
		...
		if (res := layerSource.DrawTextCanvas(flpTopLeft, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerOperand.DrawTextCanvas(flpTopLeft, 'Operand Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpTopLeft, 'Destination Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
