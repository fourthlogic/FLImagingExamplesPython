...
def main():
	...
	while True:
		...
		# Color Sequence Converter 객체 생성 // Create Color Sequence Converter object
		colorSequenceConverter = CColorSequenceConverter()

		# Source 이미지 설정 // Set the source image
		colorSequenceConverter.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 // Set the destination image
		colorSequenceConverter.SetDestinationImage(fliDestinationImage)

		# 변환할 컬러 시퀀스 입력(BGR -> RGB) // Set the color sequence to convert(BGR -> RGB)
		colorSequenceConverter.SetColorSequence(EColorSequence.RGB)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if (res := colorSequenceConverter.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Color Sequence Converter.')
			break
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image(BGR)', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpPoint, 'Destination Image(RGB)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
