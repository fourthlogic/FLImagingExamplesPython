...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationShift/Flower.flif')).IsFail():
		...
		if (res := layersDestination[0].DrawTextCanvas(flpPoint, 'Destination1 Image(left 1)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')

		if (res := layersDestination[1].DrawTextCanvas(flpPoint, 'Destination2 Image(right 2)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
