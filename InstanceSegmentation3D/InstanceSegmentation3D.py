...
def main():
	...
	while True:
		...
		if (res := viewImageDepthMap.SetImagePtr(fliDepthMapImage)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.')
			break
		...
		# 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [0], ... [n-1] 형태로 tuple 을 반환한다. # A function that receives parameters returns a tuple structured as [return], [0], ... [n-1].
		if (res := viewImageLearn.SynchronizePointOfView(viewImageDepthMap)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize view.')
			break

		# 두 이미지 뷰의 시점을 동기화 한다 # Synchronize the viewpoints of the two image views
		# 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [0], ... [n-1] 형태로 tuple 을 반환한다. # A function that receives parameters returns a tuple structured as [return], [0], ... [n-1].
		if (res := viewImageLearn.SynchronizePointOfView(viewImagesBoxContour)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize view.')
			break

		# 두 이미지 뷰 윈도우의 위치를 맞춤 # Synchronize the positions of the two image view windows
		# 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [0], ... [n-1] 형태로 tuple 을 반환한다. # A function that receives parameters returns a tuple structured as [return], [0], ... [n-1].
		if (res := viewImageLearn.SynchronizeWindow(viewImageDepthMap)[0]).IsFail():
		...
		# 두 이미지 뷰 윈도우의 위치를 맞춤 # Synchronize the positions of the two image view windows
		# 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [0], ... [n-1] 형태로 tuple 을 반환한다. # A function that receives parameters returns a tuple structured as [return], [0], ... [n-1].
		if (res := viewImageLearn.SynchronizeWindow(view3DResult)[0]).IsFail():
			ErrorPrint(res, 'Failed to synchronize window.')
			break
		...
		layerValidation = viewImageDepthMap.GetLayer(0)
		...
		viewImageDepthMap.Invalidate(True)
		...
		def Learn_thread():
			...
			eLearnResult = instanceSegmentation3DDL.Learn()
			bTerminated = True
		...
		while not instanceSegmentation3DDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = instanceSegmentation3DDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			i32MiniBatchCount = instanceSegmentation3DDL.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 # Get the last number of mini batch iterations
			i32Iteration = instanceSegmentation3DDL.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 # Get the last epoch learning
			i32Epoch = instanceSegmentation3DDL.GetLastEpoch()
			
			# 미니 배치 반복이 완료되면 cost와 validation 값을 디스플레이 
			# Display cost and validation value if iterations of the mini batch is completed 
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				# 마지막 학습 결과 비용 받기 # Get the last cost of the learning result
				f32CurrCost = instanceSegmentation3DDL.GetLearningResultLastCost()
				...
				instanceSegmentation3DDL.GetLearningResultAllHistory(listCostHistory, listMeanAPHistory, listValidationEpoch, listRecall, listPrecision)
				...
				if((listCostHistory.Count != 0 and i32PrevCostCount != listCostHistory.Count) or (listMeanAPHistory.Count != 0 and i32PrevValidationCount != listMeanAPHistory.Count)):
					i32Step = instanceSegmentation3DDL.GetLearningValidationStep()
					listX = List[Single]()
			...
			if(instanceSegmentation3DDL.IsRunning() == False):
				break
		...
		eFigureOption = Enum.ToObject(CInstanceSegmentation3DDL.EInferenceResultItemSettings, int(CInstanceSegmentation3DDL.EInferenceResultItemSettings.ClassNum) | int(CInstanceSegmentation3DDL.EInferenceResultItemSettings.ClassName) | int(CInstanceSegmentation3DDL.EInferenceResultItemSettings.Objectness))
