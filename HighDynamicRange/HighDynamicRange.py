...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/HighDynamicRange')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# 첫 페이지를 선택한다 // Select the first page
		...
		# 이미지 뷰의 썸네일을 고정한다 // Fix the thumbnail of the image view
		viewImageSrc.SetFixThumbnailView(True)

		# HighDynamicRange 객체 생성 // Create HighDynamicRange object
		highDynamicRange = CHighDynamicRange()
		...
		if (res := highDynamicRange.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute HighDynamicRange.')
			break
