...
def main():
	...
	while True:
		...
		midpointFilter = CMidpointFilter()

		# Source 이미지 설정 # Set the source image
		midpointFilter.SetSourceImage(fliSourceImage)

		# Destination 이미지 설정 # Set the destination image
		midpointFilter.SetDestinationImage(fliDestinationImage)
		
		# 처리할 Filter의 커널 크기 설정 # Set the kernel size of the filter
		midpointFilter.SetKernel(5)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if (res := midpointFilter.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Midpoint Filter.')
			break
