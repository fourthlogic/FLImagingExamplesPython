...
def main():
	...
	while True:
		...
		if (res := viewImage.Create(100, 0, 550, 480)).IsFail():
		...
		if (res := layer.DrawTextCanvas(flpPoint, 'Learn & Recognition Image', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
		...
		for i in range(i64LearnedCount):
			...
			if (res := layer.DrawTextImage(CFLPoint[float](flrBoundary.left, flrBoundary.top), flsResultString, EColor.YELLOW, EColor.BLACK, 15, False, 0, EGUIViewImageTextAlignment.LEFT_BOTTOM)).IsFail():
				ErrorPrint(res, 'Failed to draw recognized character : {0}'.format(i))
				break

			if (res := layer.DrawFigureImage(flfLearned, EColor.LIME, 1, EColor.LIME, EGUIViewImagePenStyle.Solid, 1.0, 0.35)).IsFail():
				ErrorPrint(res, 'Failed to draw recognized character : {0}'.format(i))
				break

		# Recognition 이미지 설정 # Set the recognition image
		ocr.SetSourceImage(fliImage)

		# 인식할 문자의 각도 범위를 설정
		ocr.SetRecognizingAngleTolerance(6.0)

		# 오토 스케일 여부를 설정
		ocr.EnableRecognizingAutoScale(False)

		# 인식할 문자의 최소 직사각 너비를 설정
		ocr.SetRecognizingMinimumRectangleWidth(20.0)

		# 인식할 문자의 최소 직사각 높이를 설정
		ocr.SetRecognizingMinimumRectangleHeight(40.0)

		# 인식할 문자의 최대 직사각 너비를 설정
		ocr.SetRecognizingMaximumRectangleWidth(40.0)

		# 인식할 문자의 최대 직사각 높이를 설정
		ocr.SetRecognizingMaximumRectangleHeight(60.0)

		# 인식할 최소 점수를 설정
		ocr.SetRecognizingMinimumScore(0.5)

		# 인식할 최대 개수를 설정
		ocr.SetRecognizingMaximumCharacterCount(100)
		...
		for i in range(i64ResultCount):
			...
			if (res := layer.DrawTextImage(CFLPoint[float](flrBoundary.left, flrBoundary.top), flsResultString, EColor.YELLOW, EColor.BLACK, 12, False, 0, EGUIViewImageTextAlignment.LEFT_BOTTOM)).IsFail():
				ErrorPrint(res, 'Failed to draw recognized character : {0}'.format(i))
				break

			if (res := layer.DrawFigureImage(resultChar.flfaCharacter, EColor.LIME, 1, EColor.LIME, EGUIViewImagePenStyle.Solid, 1.0, 0.35)).IsFail():
				ErrorPrint(res, 'Failed to draw recognized character : {0}'.format(i))
				break
