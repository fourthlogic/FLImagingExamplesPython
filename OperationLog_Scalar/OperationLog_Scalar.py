...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationLog/gradation_Horizontal.flif')).IsFail():
		...
		if (res := layerDestination.DrawTextCanvas(flpPoint, 'Destination Image(Log 2)', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
