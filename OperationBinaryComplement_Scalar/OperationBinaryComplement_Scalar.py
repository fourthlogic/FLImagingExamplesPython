...
def main():
	...
	while True:
		...
		if (res := fliSrcImage.Load('../../ExampleImages/OperationBinaryComplement/gradation.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# 이미지 뷰 생성 # Create image view
		if (res := viewImageSrc.Create(100, 0, 612, 512)).IsFail() or \
			(res := viewImageDst.Create(612, 0, 1124, 512)).IsFail():
