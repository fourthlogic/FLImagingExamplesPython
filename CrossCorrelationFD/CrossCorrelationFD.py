...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/CrossCorrelationFD/Cross Correlation Source.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Operand 이미지 로드 # Load the operand image
		if (res := fliOperandImage.Load('../../ExampleImages/CrossCorrelationFD/Cross Correlation Operand.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# Cross Correlation FD 객체 생성 # Create Cross Correlation FD object
		crossCorrelationFD = CCrossCorrelationFD()

		# Source 이미지 설정 # Set the source image
		crossCorrelationFD.SetSourceImage(fliSourceImage)

		# Operand 이미지 설정 # Set the operand image
		crossCorrelationFD.SetOperandImage(fliOperandImage)

		# Destination 이미지 설정 # Set the destination image
		crossCorrelationFD.SetDestinationImage(fliDestinationImage)

		# 출력 방식 설정 # Set the output method
		crossCorrelationFD.EnableNormalizedCrossCorrelation(True)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if (res := crossCorrelationFD.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute Cross Correlation FD.')
			break
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
			(res := layerOperand.DrawTextCanvas(flpPoint, 'Operand Image', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
