...
def main():
    ...
    dataMatrixVerifier = CDataMatrixVerifier()
    ...
    i64Results = dataMatrixVerifier.GetResultCount()

    for i in range(i64Results):
        # DataMatrix Verifier 결과를 얻어오기 위해 FLQuadD 선언 # Declare FLQuadD to retrieve the result from the DataMatrix Verifier.
        flqRegion = CFLQuad[Double]()

        # DataMatrix Verifier 결과들 중 Data Region 을 얻어옴 # Gets the Data Region from the DataMatrix Verifier results.
        if (res := dataMatrixVerifier.GetResultDataRegion(i, flqRegion)[0]).IsFail():
        ...
        if (res := dataMatrixVerifier.GetResultGridRegion(i, flfaGridRegion)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
        
        strDecodedMsg = StringBuilder()
        # DataMatrix Verifier 결과들 중 Decoded String 을 얻어옴 # Gets the decoded string from the results of the DataMatrix Verifier.
        if (res := dataMatrixVerifier.GetResultDecodedString(i, strDecodedMsg)[0]).IsFail():
            ErrorPrint(res, "Failed to get data region from the data matrix decoder object.")
            return
              
        print("No. {} : {}".format(i, strDecodedMsg))
        
        # DataMatrix의 결과를 디스플레이 한다. # Display the result of the DataMatrix.
        ...
        printQuality = CPrintQuality_ISOIEC_15415()
        
        if (res := dataMatrixVerifier.GetResultPrintQuality_ISOIEC_15415(i, printQuality)[0]).IsFail():
