...
def main():
	...
	while True:		
		...
		if(res := vertexMatch3D.Save("../../ExampleImages/VertexMatch3D/Example FLVM Data.flvm")).IsFail() :
			ErrorPrint(res, "Failed to save the learnt data file.\n")
			break		

		# 학습 데이터 불러오기 # Load the learnt data
		if(res := vertexMatch3D.Load("../../ExampleImages/VertexMatch3D/Example FLVM Data.flvm")).IsFail() :
