...
def main():
	...
	while True:
		...
		for i in range (0, int(EType.ETypeCount)):
			...
			if (res := (listViewImage[i].Create(i * 400 + 400, 0, i * 400 + 400 + 400, 400))).IsFail():
		...
		flrROI = CFLRect[int](20, 100, 420, 500)

		# Roberts Filter 객체 생성 // Create Roberts Filter object
		robertsFilter = CRobertsFilter()
		# Source 이미지 설정 // Set the source image
		robertsFilter.SetSourceImage(listFliImage[int(EType.Source)])
		# Source ROI 설정 // Set the Source ROI
		robertsFilter.SetSourceROI(flrROI)

		# Destination1 이미지 설정 // Set the destination1 image
		robertsFilter.SetDestinationImage(listFliImage[int(EType.Destination1)])
		...
		if (res := (listLayer[int(EType.Destination1)].DrawTextCanvas(flpZero, "Destination1 Image (Roberts)", EColor.YELLOW, EColor.BLACK, 20))).IsFail():
			ErrorPrint(res, "Failed to draw text.\n")
			break
