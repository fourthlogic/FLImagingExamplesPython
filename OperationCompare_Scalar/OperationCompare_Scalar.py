...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationCompare/nature.flif')).IsFail():
		...
		if (res := layerDestination.DrawTextCanvas(flpPoint, 'Destination Image(compare 110,110,110)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
