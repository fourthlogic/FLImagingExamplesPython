...
def main():
	...
	# GenICam 카메라 선언 # Declare the GenICam camera
	camGenICam = CDeviceCameraGenICam()
	...
	while True:
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camGenICam.GetAutoDetectGigECameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.IEEE1394):
				er = camGenICam.GetAutoDetectIEEE1394CameraSerialNumbers(listSerialNumbers)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camGenICam.GetAutoDetectUSBCameraSerialNumbers(listSerialNumbers)
		...
		if(bAutoDetect):
			...
			if(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.GigE):
				er = camGenICam.AutoDetectGigECamera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.IEEE1394):
				er = camGenICam.AutoDetectIEEE1394Camera(i32SelectDevice)
			elif(eDeviceType == CDeviceGenICamTypeBase.EDeviceType.USB):
				er = camGenICam.AutoDetectUSBCamera(i32SelectDevice)
		...
		if((er := camGenICam.Initialize()).IsFail()):
