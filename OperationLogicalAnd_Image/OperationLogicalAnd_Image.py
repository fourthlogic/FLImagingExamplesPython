...
def main():
	...
	while True:
		...
		if (res := viewImageOpr.Create(600, 0, 1100, 500)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break

		# Destination 이미지 뷰 생성 # Create the destination image view
		if (res := viewImageDst.Create(1100, 0, 1600, 500)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		operationLogicalAnd.SetOperandImage(fliOperandImage)
		
		# Destination 이미지 설정 # Set the destination image
		operationLogicalAnd.SetDestinationImage(fliDestinationImage)
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerOperand.DrawTextCanvas(flpPoint, 'Operand Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
