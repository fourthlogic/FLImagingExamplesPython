...
def main():
	...
	while True:
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'SOURCE', EColor.YELLOW, EColor.BLACK, 30)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpPoint, 'RESULT', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		# UnnamedLabelRemoverDL 객체 생성 # Create UnnamedLabelRemoverDL object
		unnamedLabelRemoverDL = CUnnamedLabelRemoverDL()

		# Source 이미지 설정 # Set the source image
		unnamedLabelRemoverDL.SetSourceImage(fliSourceImage)
		# Destination 이미지 설정 # Set the destination image
		unnamedLabelRemoverDL.SetDestinationImage(fliDestinationImage)

		# 앞서 설정된 파라미터 대로 알고리즘 수행 # Execute algorithm according to previously set parameters
		if (res := unnamedLabelRemoverDL.Execute()).IsFail():
			ErrorPrint(res, 'Failed to execute UnnamedLabelRemoverDL.')
			break
