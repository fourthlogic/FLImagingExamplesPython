...
def main():
	...
	viewSource2Image = CGUIViewImage()
	...
	while True:
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewSource2Image.SetImagePtr(fliSource2Image)[0]).IsFail():
			ErrorPrint(res, 'Failed to set image object on the image view.\n')
			break
		...
		# ref 파라미터를 입력 받는 함수는 리턴이 tuple로 생성되며 [return], [ref 0], ... [ref n-1] 형태로 tuple 을 반환한다. # A function that receives ref parameters returns a tuple structured as [return], [ref 0], ... [ref n-1].
		if (res := viewSourceImage.SynchronizeWindow(viewSource2Image)[0]).IsFail():
		...
		layerSrc = viewSourceImage.GetLayer(0)
		layerSrc2 = viewSource2Image.GetLayer(0)
		layerDst = viewDestinationImage.GetLayer(0)
		...
		if (res := layerSrc.DrawTextCanvas(CFLPoint[Double](0, 0), 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.\n')
			break
		
		if (res := layerSrc2.DrawTextCanvas(CFLPoint[Double](0, 0), 'Source Image 2', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.\n')
			break
		
		if (res := layerDst.DrawTextCanvas(CFLPoint[Double](0, 0), 'Destination Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.\n')
			break
		...
		while viewSourceImage.IsAvailable() and viewSource2Image.IsAvailable() and viewDestinationImage.IsAvailable() and view3DDst.IsAvailable():
