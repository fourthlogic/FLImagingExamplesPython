...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/LinearInference/LinearInference_Src.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Operand 이미지 로드 # Load the operand image
		if (res := fliOperandImage.Load('../../ExampleImages/LinearInference/LinearInference_Opr.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		linearInference = CLinearInference()

		# Source 이미지 설정 # Set the source image
		linearInference.SetSourceImage(fliSourceImage)

		# Operand 이미지 설정 # Set the operand image
		linearInference.SetOperandImage(fliOperandImage)

		# Destination 이미지 설정 # Set the destination image
		linearInference.SetDestinationImage(fliDestinationImage)
