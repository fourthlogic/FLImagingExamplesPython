...
def main():
	...
	while True:
		...
		if (res := arrFliImage[EType.Source].Load('../../ExampleImages/ImageThreshold/SunSrc.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Operand 이미지 로드 # Load the operand image
		if (res := arrFliImage[EType.Operand].Load('../../ExampleImages/ImageThreshold/SunOpr.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		# 객체 생성 # Create object
		imageThreshold = CImageThreshold()

		# Source 이미지 설정 # Set the source image
		imageThreshold.SetSourceImage(arrFliImage[EType.Source])

		# Operand 이미지 설정 # Set the operand image
		imageThreshold.SetOperandImage(arrFliImage[EType.Operand])

		# Destination 이미지 설정 # Set the destination image
		imageThreshold.SetDestinationImage(arrFliImage[EType.Destination])

		# 임계값 오프셋 설정 # Set the threshold offset
		imageThreshold.SetThresholdOffset(CMultiVar[Double](5))
