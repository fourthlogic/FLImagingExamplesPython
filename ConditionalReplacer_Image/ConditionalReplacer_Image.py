...
def main():
	...
	while True:
		...
		if (res := arrFliImage[EType.Source].Load('../../ExampleImages/ConditionalReplacer/CatSource.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break

		# Operand 이미지 로드 // Load the operand image
		if (res := arrFliImage[EType.Operand].Load('../../ExampleImages/ConditionalReplacer/CatOperand.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		...
		conditionalReplacer.SetOperandImage(arrFliImage[EType.Operand])
		...
		mvReplacement = CMultiVar[Double](0)
		...
		if (res := arrLayer[EType.Operand].DrawTextCanvas(flpPoint, 'Operand Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
