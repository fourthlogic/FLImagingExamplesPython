...
def main():
	...
	# XMC 선언 # Declare XMC
	motionXMC = CDeviceMotionXMC()

	while True:
		# 장치가 연결된 IP 주소를 입력합니다. # Connected devices ip address.
		strInput = input("IP : ")
		...
		motionAxis : CDeviceMotionAxisXMC = motionXMC.GetMotionAxis(0)
		...
		while True:
			...
			if eMotionFeature == EMotionFeature.SearchOriginPosition:
				...
				while motionAxis.IsInMotion() == False:
					CThreadUtilities.Sleep(100)

				if motionAxis.IsMotionDone() == False:
				...
				if eMotionFeature == EMotionFeature.MoveAbsolutePosition:
					...
					while motionAxis.IsInMotion() == True:
						CThreadUtilities.Sleep(100)
