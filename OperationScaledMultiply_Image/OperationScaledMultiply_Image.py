...
def main():
	...
	while True:
		...
		if (res := fliSourceImage.Load('../../ExampleImages/OperationScaledMultiply/Generator.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
		
		# Operand 이미지 로드 # Load the operand image
		if (res := fliOperandImage.Load('../../ExampleImages/OperationScaledMultiply/Gradation_R2W.flif')).IsFail():
			ErrorPrint(res, 'Failed to load the image file.')
			break
