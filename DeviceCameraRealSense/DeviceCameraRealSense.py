...
class CDeviceEventImageEx(CDeviceEventImageBase):
	...
	def OnAcquisition(self, deviceImage):
		while(True):
			...
			if(not isinstance(deviceImage, CDeviceCameraRealSense)):
				break
...
def main():
	...
	while True:
		...
		while True:
			strMajor = input("Enter Major Version: ")
			strMinor = input("Enter Minor Version: ")
			...
			if(strMajor.isdigit() and strMinor.isdigit() and strPatch.isdigit()):
				...
				i32Patch = int(strPatch)

				if ((er := camRealSense.SetModuleVersion(i32Major, i32Minor, i32Patch)).IsFail()):
					bValid = False
					print("Incorrect version. Please enter again.")
