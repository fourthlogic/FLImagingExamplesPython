...
def main():
	...
	while True:
		...
		if (res := arrLayer[EType.Destination2].DrawTextCanvas(flpPoint, 'Destination2 Image (Sigma 1)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination3].DrawTextCanvas(flpPoint, 'Destination3 Image (Sigma 3)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination4].DrawTextCanvas(flpPoint, 'Destination4 Image (Sigma 5)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
