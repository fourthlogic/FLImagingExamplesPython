...
def main():
	...
	while True:
		...
		def Learn_thread():
			...
			eLearnResult = instanceSegmentation.Learn()
			bTerminated = True
		...
		while not instanceSegmentation.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = instanceSegmentation.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			i32MiniBatchCount = instanceSegmentation.GetActualMiniBatchCount()
			# 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
			i32Iteration = instanceSegmentation.GetLearningResultCurrentIteration()
			# 마지막 학습 횟수 받기 // Get the last epoch learning
			i32Epoch = instanceSegmentation.GetLastEpoch()
			
			# 미니 배치 반복이 완료되면 cost와 validation 값을 디스플레이 
			# Display cost and validation value if iterations of the mini batch is completed 
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				# 마지막 학습 결과 비용 받기 // Get the last cost of the learning result
				f32CurrCost = instanceSegmentation.GetLearningResultLastCost()
				...
				instanceSegmentation.GetLearningResultAllHistory(listCostHistory, listMeanAPHistory, vctValidationEpoch)
				...
				if((listCostHistory.Count != 0 and i32PrevCostCount != listCostHistory.Count) or (listMeanAPHistory.Count != 0 and i32PrevValidationCount != listMeanAPHistory.Count)):
					i32Step = instanceSegmentation.GetLearningValidationStep()
					listX = List[Single]()
			...
			if(instanceSegmentation.IsRunning() == False):
				break
		...
		instanceSegmentation.SetInferenceResultObjectnessThreshold(0.5)
		# Mask Threshold 설정 // Set The mask threshold
		instanceSegmentation.SetInferenceResultMaskThreshold(0.5)
