...
def main():
	...
	while True:		
		...
		if(res := fliSource.Load("../../ExampleImages/XYZImageToPointCloudConverter3D/XYZV.flif")).IsFail() :		
			ErrorPrint(res, "Failed to load the image.\n")
			break

		if(res := fliTexture.Load("../../ExampleImages/XYZImageToPointCloudConverter3D/Texture.flif")).IsFail() :		
			ErrorPrint(res, "Failed to load the image.\n")
			break
		...
		xyzImageToPointCloudConverter3D.SetTextureImage(fliTexture)
		...
		if(res := view3D.PushObject(floDestination)).IsFail() :		
