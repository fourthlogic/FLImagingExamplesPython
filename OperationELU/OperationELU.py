...
def main():
	...
	while True:
		...
		operationELU = COperationELU()
		
		if (res := operationELU.SetSourceImage(fliSrcImage)[0]).IsFail():
			break
		if (res := operationELU.SetDestinationImage(fliDstImage0)[0]).IsFail():
			break
		...
		if (res := operationELU.SetDestinationImage(fliDstImage1)[0]).IsFail():
			break
