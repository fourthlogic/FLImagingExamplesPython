...
def main():
	...
	# Flip 방향 선언 // Declare filp direction
	eDirection = [CFlip.EDirection.Horizontal, CFlip.EDirection.Vertical, CFlip.EDirection.Both]

	while True:
		
		# Source 이미지 로드 // Load the source image
		if (res := fliSourceImage.Load('../../ExampleImages/Flip/Deer.flif')).IsFail():
		...
		if (res := viewImageSrc.Create(612, 0, 1124, 512)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		
		# Destination 이미지 뷰 생성 // Create the destination image view
		if (res := viewImageDst[0].Create(100, 512, 612, 1024)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		if (res := viewImageDst[2].Create(1124, 512, 1636, 1024)).IsFail():
			ErrorPrint(res, 'Failed to create the image view.')
			break
		...
		# Flip 객체 생성 // Create Flip object
		flip = CFlip()

		# Source 이미지 설정 // Set the source image
		flip.SetSourceImage(fliSourceImage)
		...
		layerDestination = [viewImageDst[0].GetLayer(0), viewImageDst[1].GetLayer(0), viewImageDst[2].GetLayer(0)]
		...
		if (res := layerDestination[0].DrawTextCanvas(flpPoint, 'Destination Image Horizontal', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		
		if (res := layerDestination[1].DrawTextCanvas(flpPoint, 'Destination Image Vertical', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := layerDestination[2].DrawTextCanvas(flpPoint, 'Destination Image Both', EColor.YELLOW, EColor.BLACK, 30)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
		...
		while viewImageSrc.IsAvailable() and viewImageDst[0].IsAvailable() and viewImageDst[1].IsAvailable() and viewImageDst[2].IsAvailable():
