...
def main():
	...
	while True:
		...
		conditionalExtractor.SetThreshold(100)
		conditionalExtractor.SetThreshold(128, EThresholdIndex.Second)

		# 논리조건 설정 # Set the logical condition
		conditionalExtractor.SetLogicalCondition(ELogicalCondition.Less)
		conditionalExtractor.SetLogicalCondition(ELogicalCondition.NotEqual, EThresholdIndex.Second)
		...
		mvOutOfRange = CMultiVar[Double](255)
		...
		if (res := layerSource.DrawTextCanvas(flpPoint, 'Source Image', EColor.YELLOW, EColor.BLACK, 20)).IsFail() or \
			(res := layerDestination.DrawTextCanvas(flpPoint, 'Destination Image\nDual And(Less than 100 and Not Equal 128)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
