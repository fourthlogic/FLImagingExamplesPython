...
def main():
	...
	while True:
		...
		if (res := arrLayer[EType.Destination1].DrawTextCanvas(flpPoint, 'Destination1 Image (Kernel Size 3x3)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination2].DrawTextCanvas(flpPoint, 'Destination2 Image (Kernel Size 5x5)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination3].DrawTextCanvas(flpPoint, 'Destination3 Image (Kernel Size 7x7)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break

		if (res := arrLayer[EType.Destination4].DrawTextCanvas(flpPoint, 'Destination4 Image (Kernel Size 15x15)', EColor.YELLOW, EColor.BLACK, 20)).IsFail():
			ErrorPrint(res, 'Failed to draw text.')
			break
