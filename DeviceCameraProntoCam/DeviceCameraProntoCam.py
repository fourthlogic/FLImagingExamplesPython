...
def main():
	...
	# ProntoCam 카메라 선언 // Declare the ProntoCam camera
	camProntoCam = CDeviceCameraProntoCam()
	...
	while True:
		...
		if(bAutoDetect):
			...
			er = camProntoCam.GetAutoDetectCameraSerialNumbers(listSerialNumbers)
		...
		if(bAutoDetect):
			...
			if((er := camProntoCam.AutoDetectCamera(i32SelectDevice)).IsFail()):
		...
		if((er := camProntoCam.Initialize()).IsFail()):
		...
		if((er := camProntoCam.Live()).IsFail()):
