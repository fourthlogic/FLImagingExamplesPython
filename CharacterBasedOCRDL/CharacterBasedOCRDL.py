...
def main():
	...
	while True:
		...
		# 학습을 종료할 조건식 설정. mAP값이 0.85 이상인 경우 학습 종료한다. metric과 동일한 값입니다.
		# Set Conditional Expression to End Learning. If the mAP value is 0.85 or higher, end the learning. Same value as metric.ze.
		...
		while not characterBasedOCRDL.IsRunning() and not bTerminated:
			time.sleep(0.001)

		i32MaxEpoch = characterBasedOCRDL.GetLearningEpoch()
		i32PrevEpoch = 0
		i32PrevCostCount = 0
		...
		while(True):
			...
			if i32Epoch != i32PrevEpoch and i32Iteration == i32MiniBatchCount and i32Epoch > 0:
				...
				characterBasedOCRDL.GetLearningResultAllHistory(listCostHistory, listMeanAPHistory, listValidationEpoch, listRecall, listPrecision)
				...
				if(f32ValidationMeanAP == 1.0 or bEscape):
